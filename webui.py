...
parser.add_argument("--use_accel", action="store_true", default=False, help="whether to use acceleration engine for GPT2 or not.")
parser.add_argument("--use_torch_compile", action="store_true", default=False, help="whether to use torch.compile for optimization or not.")
...
def gen_single(emo_control_method,prompt, text,
               ...
               emo_text,emo_random,
               verbose_checkbox=cmd_args.verbose,
               max_text_tokens_per_segment=120,
               save_file=True,
               stream_checkbox=False,
...
<a href='https://arxiv.org/abs/2506.21619'><img src='https://img.shields.io/badge/ArXiv-2506.21619-red'></a>
    ...
    with gr.Tab(i18n("音频生成")):
        with gr.Row():
            ...
            output_audio = gr.Audio(label=i18n("生成结果"),streaming=True,autoplay=True,visible=True)

        with gr.Row():
            experimental_checkbox = gr.Checkbox(label=i18n("显示实验功能"), value=False)
            verbose_checkbox = gr.Checkbox(label="控制台输出详细信息", value=cmd_args.verbose)
            save_file = gr.Checkbox(label="保存文件", info="默认保存在根目录outputs文件夹", value=True)
            stream_checkbox = gr.Checkbox(label="流式播放", info="一边生成一边播放。", value=False)
    ...
    def on_stream_change(is_stream):
        if is_stream:
           gr.Warning(i18n("已启用自动播放和流式生成！"))
           return gr.update(label=i18n("流式生成"))
        else:
           gr.Warning(i18n("已关闭自动播放和流式生成！"))
           return gr.update(label=i18n("生成结果"))
