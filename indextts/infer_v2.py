...
class IndexTTS2:
    ...
    def interval_silence(self, wavs, sampling_rate=22050, interval_silence=200):
        ...
        return torch.zeros(channel_size, sil_dur, dtype=torch.float32)
    ...
    def insert_interval_silence(self, wavs, sampling_rate=22050, interval_silence=200):
        ...
        # get silence tensor
        sil_dur = int(sampling_rate * interval_silence / 1000.0)
        sil_tensor = torch.zeros(channel_size, sil_dur, dtype=torch.float32)
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path,
        ...
        for seg_idx, sent in enumerate(segments):
            ...
            with torch.no_grad():
                ...
                if output_path:
                    ...
                    wav = torch.clamp(32768 * wav, -32768.0, 32767.0)
                if verbose:
                ...
                if stream_return:
                    ...
                    combined = torch.cat([wav.cpu(), silence], dim=1)
                    if output_path:
                        combined = torch.clamp(32768 * combined, -32768.0, 32767.0)
                    wav_data = combined.type(torch.int16).cpu()
        ...
        if output_path:
            ...
            torchaudio.save_with_torchcodec(uri=output_path, src=wav.type(torch.float32), sample_rate=sampling_rate)
